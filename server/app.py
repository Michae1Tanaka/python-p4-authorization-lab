...
class MemberOnlyIndex(Resource):
    def get(self):
        if not session.get("user_id"):
            response_body = {"message": "You must log in to view these article."}
            response = make_response(response_body, 401)
            return response
        elif session.get("user_id"):
            member_only_articles = Article.query.filter(
                Article.is_member_only == True
            ).all()
            article_dict = [article.to_dict() for article in member_only_articles]
...
class MemberOnlyArticle(Resource):
    def get(self, id):
        if not session.get("user_id"):
            response_body = {"message": "You must log in to view this article."}
            response = make_response(response_body, 401)
            return response
        elif session.get("user_id"):
            article = Article.query.filter(Article.id == id).first()
